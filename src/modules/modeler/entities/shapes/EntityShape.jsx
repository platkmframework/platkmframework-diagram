...
export class EntityShape extends RectangleShape{
    ...
    constructor( ){
        ...
        this.state.shape.type = C_ENTITY_SHAPE,
...
//---------------------FUNCIONES DE MOVIMIENTO IMPLEMENTADAS POR TODAS LAS FIGURAS----------------- */
    ...
    moveSelected(ctx, selectedShapeData, shapes, mx, my){
        ...
        if(selectedShapeData.id == this.state.shape.id){
            ...
            this.state.content.map((s, index)=>{
                ...
                s.moveRequest(ctx, this, selectedShapeData, shapes, this.state.shape.x, this.state.shape.y + cHeight)
                cHeight+=20
            })
    ...
    /**
        this. drawUMLModule(ctx,
            50, 50, // Position
            300, 200, // Width and height
            "ExampleClass", // Module name
            ["- attribute1: String", "- attribute2: int"], // Properties
            ["+ method1()", "+ method2(param: type)"] // Methods
          );
     */
    drawUMLModule(ctx, selectedShapeData, shapes) {
        const headerHeight = 30;
        const propertyHeight = this.state.content.length * 20;
        ...
        ctx.fillText(this.state.shape.name, this.state.shape.x + this.state.shape.width / 2, this.state.shape.y + headerHeight / 2);
  
        // Draw properties section
        ctx.strokeRect(this.state.shape.x, this.state.shape.y + headerHeight, this.state.shape.width, propertyHeight);
        ctx.textAlign = "left";
        ctx.font = "14px Arial";
        console.log(this.state.content)
        this.state.content.forEach((csh, index) => {
            csh.draw(ctx, selectedShapeData, shapes)
        });
