...
import { C_ASYNCHRONOUS_MESSAGE, C_BASE_LINE, C_BASE_RECTANGLE, C_DEFAULT_BORDER_MOVE_COLOR, C_DEFAULT_CONNECTION_COLOR} from "../../constants/ShapeTypes";
...
export class LineShapeCopy{
    ...
    constructor(id,sourceId, targetId, arrow, orientation, x1,y1, x2, y2){
        this.state = {
            shape: {
                ...
                recursiveline:false,
                acceptedIncomingType:[C_BASE_RECTANGLE],
                acceptedOutgoingType:[C_BASE_RECTANGLE],
                mousestyle:'move',
                color:'black',
                relateExeSpecY:'', 
                containerParentId:'',
     ...
     */
    draw(ctx, selectedShapeData, shapes, lines){
        ctx.beginPath();  
        ctx.fillStyle = 'black'
...
/* 
        ...
        if(!isSourcerId){
            //find new source 
            for (let index = 0; index < shapes.length; index++) {
                newSourceElem = shapes[index].findByCoordinatesAndAcceptanceType(ctx,this.state.shape.points[0].x, this.state.shape.points[0].y, shapes, flowLines, C_ASYNCHRONOUS_MESSAGE, false, [])
        ...
        if(!isTargetId){
            //find new target 
            for (let index = 0; index < shapes.length; index++) {
                newTargetElem = shapes[index].findByCoordinatesAndAcceptanceType(ctx,this.state.shape.points[1].x, this.state.shape.points[1].y, shapes, flowLines, C_ASYNCHRONOUS_MESSAGE, false, [])
     ...
     * @returns 
     */
    moveSelected(ctx, selectedShapeData, shapes, flowLines, mx, my){
...
/*         this.state.shape.points[0].y = my
     ...
     * @param {*} element 
     */
    moveRequest(ctx, element, selectedShapeData, shapes, flowLines, mx, my){ 
/*         if(element.state.shape.id == this.state.shape.sourceId){
     ...
     * @returns 
     */
    findByCoordinatesAndAcceptanceType(ctx,mx, my, shapes, lines, incomingType, byContainer, excluded){
    ...
    remove(data){
        ...
        //se elimina el mismo
        let linesList = data.lines.filter(l=> l.getShape().id != this.getShape().id)

        return {shapes:data.shapes, lines:linesList}
    }
