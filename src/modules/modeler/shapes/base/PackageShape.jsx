...
//---------------------FUNCIONES DE MOVIMIENTO IMPLEMENTADAS POR TODAS LAS FIGURAS----------------- */
    ...
    draw(ctx, selectedShapeData, shapeList){  
       this. drawUMLPackage(ctx,
            50, 50, // Position
            300, 200, // Width and height
            "ExamplePackage" // Package name
          );
    }
    ...
    drawUMLPackage(ctx, x, y, width, height, packageName) {
        const tabWidth = 120;
        const tabHeight = 30;
  
        // Draw tab (header rectangle)
        ctx.fillStyle = "#d0e0ff";
        ctx.fillRect(x, y, tabWidth, tabHeight);
        ctx.strokeRect(x, y, tabWidth, tabHeight);
  
        // Add package name in the tab
        ctx.fillStyle = "#000";
        ctx.font = "14px Arial";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(packageName, x + tabWidth / 2, y + tabHeight / 2);
  
        // Draw main rectangle for package body
        ctx.strokeRect(x, y + tabHeight, width, height - tabHeight);
      }
